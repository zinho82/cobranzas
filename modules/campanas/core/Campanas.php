<?php
	class Campanas{ 
            ...
            public function getArchivoBD($archivo,$ruta,$ncarga,$separador){
             ...
             echo    "<br>guardando archivo temporal<br>";
              ...
              echo       $sql="select * from callcenter.archivos where idarchivos='".$archivo."'";
              ...
              foreach ($lineas as $linea_num => $linea)
                {
                 $datos = explode("\t",$linea);
                    ...
                    mysql_query($consulta, $ObjConfig->conectar_db('callcenter')) or die(mysql_error());
                }
            ...
            public function traer_archivo($tipo_carga){
                ...
                echo $sql="select * from callcenter.archivos where proceso='".$tipo_carga."' and date(fecha_upload) like '".date('Y-m')."%'";
                $res=mysql_query($sql,$ObjConfig->conectar_db('callcenter')) or die(mysql_error());
                while($archi=  mysql_fetch_array($res)){
                    echo "<option value='".$archi['idarchivos']."'>".$archi['nombre']." (".$archi['fecha_upload'].")"."</option>";
                }
            }
            public function CargarCampana($bd,$tabla,$numcarga) {
                $ObjConfig = new Config();
        ...
        mysql_query($sql,  $ObjConfig->conectar_db($bd)) or die(mysql_error());
                if(mysql_affected_rows()>0){
            ...
            public function RevisarNumeroTelefono($bd,$tabla,$campotelefono,$area,$telefono) {
                ...
                $this->RepararFonos($area,$telefono,$bd,$tabla);
            ...
            public function getCampana($idcampana) {
                 $ObjConfig = new Config();
          echo       $sql="select * from callcenter.campanas where idcampanas=".$idcampana;
                $res=mysql_query($sql, $ObjConfig->conectar_db('callcenter')) or die(mysql_error());
            ...
            public function ListaCampanasSelect(){
                $ObjConfig=new Config();
                echo "<option selected=''>Seleccione una Campaña</option>";
            ...
            public function ListaAgentesSelect(){
                  $ObjConfig=new Config();
                echo "<option selected>Seleccione un agente</option>";
                 $sql="select * from users where perfil='agente'"; 
                $res=  mysql_query($sql, $ObjConfig->conectar_db('callcenter'))or die(mysql_error());
                while($cam=  mysql_fetch_array($res)){
                    echo '<option value="'.$cam['user_id'].'">'.$cam['name'].'</option>';
                }
            }
            public function EjecutivoAsignado() {
                  $ObjConfig=new Config();
                 $sql="select * from ejecutivo_campana";
                $res=  mysql_query($sql,  $ObjConfig->conectar_db("callcenter")) or die(mysql_error());
                while($eje=mysql_fetch_array($res)){
                    echo "<tr><td>". Campanas::Nombreagente( $eje['ejecutivo'])."</td><td>".  Campanas::NombreCampana($eje['campana'])."</td><td>".$eje['ncarga']."</td></tr>";
                }
            } 
            public function GuardaAsignacion($ejecutivo,$campana,$carga) {
                  $ObjConfig=new Config();
                $borra="delete from ejecutivo_campana where ejecutivo=$ejecutivo";
                mysql_query($borra,  $ObjConfig->conectar_db("callcenter"));
                 $sql="insert into ejecutivo_campana values($ejecutivo,$campana,'$carga')";
                if(mysql_query($sql,  $ObjConfig->conectar_db("callcenter"))){
                    echo "Guardado";
                }
                else{
                    echo "NO Guardado";
            ...
            echo      $sql="select numero_de_carga from ".$bd.".".$tabla." group by numero_de_carga order by numero_de_carga desc";
                $res=mysql_query($sql,  $ObjConfig->conectar_db($bd));
            ...
            public function MarcarCartas($bd,$tabla,$numcarga,$campoactualizar,$campo_temporal){
                ...
                mysql_query($sql,  $ObjConfig->conectar_db($bd));   

                $sql="update ".$bd.".".$tabla." datos
                    inner join callcenter.temporal tmp on tmp.c1=datos.dmacct  and trim(tmp.".$campo_temporal.")='1'
                    set datos.".$campoactualizar."='SI',datos.u6codcam=tmp.c5 and datos.numero_de_carga=$numcarga";
                mysql_query($sql,  $ObjConfig->conectar_db($bd));
            ...
            public function RepararFonos($area,$fono,$bd,$tabla) {
                  $ObjConfig=new Config();
              echo    $sql="update ".$bd.".".$tabla." da set da.".$area."= "
                       ...
                       . "when length(da.".$area.")=2 and length(da.".$fono.")=8 and da.".$area."!=2 then 9 "
                       . "when length(da.".$area.")=1 and length(da.".$fono.")=9 and da.".$area."!=2 then 0 "  
                       . "when length(da.".$area.")=1 and length(da.".$fono.")=9 and da.".$area."=2 then 0 "  
            ...
            public function ActualizarFono($area,$fono,$bd,$tabla,$fonoDestino) {
                $conn=new Config();
                 $sql="update $bd.$tabla set $fonoDestino=concat($area,$fono)";
                mysql_query($sql,$conn->conectar_db($bd));
                echo mysql_affected_rows();
            }
            public function Fonosok($fono,$bd,$tabla) {
                $conn=new Config();
                $sql="select count(*) from $bd.$tabla where length($fono)=9";
                $res=  mysql_query($sql,$conn->conectar_db($bd));
                echo mysql_result($res, 0);
            }
            public function FonosBad($fono,$bd,$tabla) {
                $conn=new Config();
                $sql="select count(*) from $bd.$tabla where length($fono)!=9";
                $res=  mysql_query($sql,$conn->conectar_db($bd));
                echo mysql_result($res, 0);
            ...
            public function ActivarRegistro($bd,$tabla,$numerodecarga) {
                  $ObjConfig=new Config();
                echo   $sql="update $bd.$tabla inner join callcenter.temporal on c1=dmacct and numero_de_carga='$numerodecarga' set estado=1 ";
                mysql_query($sql,  $ObjConfig->conectar_db($bd));
                ...
                $res=  mysql_query($sql,  $ObjConfig->conectar_db("callcenter"));
                echo "<br><i class='fa fa-check-circle'></i>  Se bloquearon ".mysql_result($res, 0)." Registros";
                    
                } 
            public function ActualizarBD($bd,$tabla,$numerocarga) {
                  $ObjConfig=new Config();
                     $sql="update $bd.$tabla da
                            inner join callcenter.temporal tmp on da.dmacct=tmp.c1 and da.numero_de_carga='$numerocarga'
 ...
 ,da.dmppdue=tmp.c55,da.dmppkept=tmp.c57,da.dmppbrok=tmp.c58,da.dmppflag=tmp.c59,da.dmppmade=tmp.c60 ";
                    mysql_query($sql,  $ObjConfig->conectar_db("callcenter"));
            ...
            public function crear_campanaNueva($campana,$idpred,$nomCampana) {
                  $ObjConfig=new Config();
                   $sql="insert into callcenter.campanaespecial values('$campana',$idpred,'$nomCampana',1)";
             mysql_query($sql,  $ObjConfig->conectar_db("callcenter")) or die(mysql_error());
             
                
            }
            public function EnviarNuevaCampana($campanaPred,$campana,$bd,$tabla,$fono,$numcarga){
                  $ObjConfig=new Config();
                   $sql="select id_cartera as idcliente, case when $fono is null or length($fono)>9 then 1 else $fono end as fono from $bd.$tabla da inner join callcenter.temporal tmp on tmp.c1=da.dmacct and da.numero_de_carga=$numcarga and da.estado=1 and da.accion!=1";
                $res=mysql_query($sql,  $ObjConfig->conectar_db($bd));
                while($num=mysql_fetch_array($res)){
                     $ssql="insert into call_center.calls (id_campaign,phone,fecha_llamada,status,uniqueid) values ('".$campanaPred."','".$num['fono']."','".date('Y-m-d G:i:s')."','".$num['idcliente']."','".$campana."')";
                  mysql_query($ssql,  Predictivo::conectar_db_predictivo("call_center")) or die(mysql_error());
                  $ids=mysql_insert_id();
                   $attrib="insert into call_center.call_attribute values(null,".$ids.",'idcliente',".$num['idcliente'].",1)";
                  mysql_query($attrib,  Predictivo::conectar_db_predictivo("call_center")) or die(mysql_error());
                   $attrib="insert into call_center.call_attribute values(null,".$ids.",'campana',".$campana.",2)";
                  mysql_query($attrib,  Predictivo::conectar_db_predictivo("call_center")) or die(mysql_error());
                }
                $sql="update call_center_calls set retries=99 where phone=1";
                mysql_query($sql,  Predictivo::conectar_db_predictivo("call_center"));
                
                
            }
            public function NombreCampana($idcampana){
                  $ObjConfig=new Config();
                $sql="select campana from campanas where idcampanas='$idcampana'";
                $result=mysql_query($sql,  $ObjConfig->conectar_db("callcenter")) or die(mysql_error());
                return mysql_result($result, 0);
                
            }
            public function NombreAgente($idAgente){
                  $ObjConfig=new Config();
                $sql="select name from users where user_id='$idAgente'";
                $result=mysql_query($sql,  $ObjConfig->conectar_db("callcenter")) or die(mysql_error());
                return mysql_result($result, 0);
