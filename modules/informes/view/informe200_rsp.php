...
inner join callcenter.resp_rapida_resultado rrs on rrs.id_resp_rapida_resultado=datos.resp_rapida_accion_id_resp_rapida_accion where date(datos.fecha_gestion) like '".str_replace('/', '-', $_POST['fecha'])."%'";
        $res=mysql_query($sql,  Config::conectar_db('callcenter')) or die(mysql_error());
                while($consu=mysql_fetch_array($res)){
                      $contenido=$consu['campo1'].  Config::espacios_izq($consu['cuenta'],25)
                            ...
                            .Config::espacios_izq(str_replace(chr(10), '',str_replace(chr(13), '',$consu['fono_contacto'].'.'.$consu['observacion'])))
                            .chr(13).chr(10);
                    fwrite($fp,$contenido);
                }
        /////////////////////
        ...
        $res=mysql_query($sql,  Config::conectar_db('callcenter')) or die(mysql_error());
                while($consu=mysql_fetch_array($res)){
                      $contenido=$consu['campo1'].  Config::espacios_izq($consu['cuenta'],25)
                            ...
                            .$consu['cod_resultado']
                            .  Config::espacios('063',10)
                            .  Config::espacios_izq(str_replace(chr(10), '',str_replace(chr(13), '',$consu['fono_contacto'].'.'.$consu['observacion'])),62)
                            .chr(13).chr(10);
                    fwrite($fp,$contenido); 
                }
            /////////////////////
